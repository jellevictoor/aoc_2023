...
class Game:
    ...
    def get_score(self):
        found_numbers = [i for i in self._winning_numbers if i in self._your_numbers]
        score = 0
...
def create_game(card):
    ...
    return Game(int(game_id), re.findall(r"(\s?[\s|\d]?\d)",winning_numbers), re.findall(r"(\s?[\s|\d]?\d)",your_numbers))


def main():
    with open('input/day4.txt', 'r') as f:
        return count_score(clean_lines(f.read()))
